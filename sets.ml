...
let UNIONS_UNIV = prove
 (`UNIONS UNIV = (UNIV : A set)`,
  REWRITE_TAC [EXTENSION; IN_UNIONS; IN_UNIV] THEN
  X_GEN_TAC `x : A` THEN
  EXISTS_TAC `UNIV : A set` THEN
  REWRITE_TAC [IN_UNIV]);;

export_thm UNIONS_UNIV;;

let UNIONS_INSERT_EMPTY = prove
 (`!(s : (A set) set). UNIONS ({} INSERT s) = UNIONS s`,
  GEN_TAC THEN
  ONCE_REWRITE_TAC [EXTENSION] THEN
  X_GEN_TAC `x : A` THEN
  REWRITE_TAC [IN_UNIONS; IN_INSERT] THEN
  EQ_TAC THENL
  [DISCH_THEN (X_CHOOSE_THEN `t : A set` STRIP_ASSUME_TAC) THENL
   [SUBGOAL_THEN `F` CONTR_TAC THEN
    UNDISCH_TAC `t = ({} : A set)` THEN
    REWRITE_TAC [GSYM MEMBER_NOT_EMPTY] THEN
    EXISTS_TAC `x : A` THEN
    ASM_REWRITE_TAC [];
    EXISTS_TAC `t : A set` THEN
...
export_thm UNIONS_INSERT_EMPTY;;

let UNIONS_DELETE_EMPTY = prove
 (`!(s : (A set) set). UNIONS (s DELETE {}) = UNIONS s`,
  GEN_TAC THEN
  ONCE_REWRITE_TAC [EXTENSION] THEN
  X_GEN_TAC `x : A` THEN
  REWRITE_TAC [IN_UNIONS; IN_DELETE] THEN
  EQ_TAC THENL
  [DISCH_THEN (X_CHOOSE_THEN `t : A set` STRIP_ASSUME_TAC) THEN
   EXISTS_TAC `t : A set` THEN
   ASM_REWRITE_TAC [];
...
export_thm UNIONS_DELETE_EMPTY;;
...
let INTERS_SUBSET = prove
 (`!u (s : A set). (?t. t IN u /\ t SUBSET s) ==> INTERS u SUBSET s`,
  REPEAT GEN_TAC THEN
  REWRITE_TAC [SUBSET; IN_INTERS] THEN
  DISCH_THEN (X_CHOOSE_THEN `t : A set` STRIP_ASSUME_TAC) THEN
  X_GEN_TAC `x : A` THEN
  DISCH_THEN (MP_TAC o SPEC `t : A set`) THEN
  ASM_REWRITE_TAC []);;
...
let IMAGE_DELETE_INJ = prove
 (`!(f : A -> B) s a.
  ...
  REPEAT STRIP_TAC THEN
  REWRITE_TAC [GSYM DELETE_DIFF_SING; GSYM IMAGE_SING] THEN
  MATCH_MP_TAC IMAGE_DIFF_INJ THEN
...
export_thm IMAGE_DELETE_INJ_ALT;;
...
let SUBSET_IMAGE_INJ = prove
 (`!(f : A -> B) s t.
  ...
  [DISCH_THEN (X_CHOOSE_THEN `u : A set` STRIP_ASSUME_TAC) THEN
   ...
   ASM_REWRITE_TAC [];
   STRIP_TAC THEN
   SUBGOAL_THEN `!x. x IN s ==> ?y. y IN t /\ (f : A -> B) y = x` MP_TAC THENL
   [X_GEN_TAC `x : B` THEN
    POP_ASSUM MP_TAC THEN
    REWRITE_TAC [SUBSET] THEN
    REPEAT STRIP_TAC THEN
    ...
    EXISTS_TAC `IMAGE (g : B -> A) s` THEN
    UNDISCH_TAC `s SUBSET IMAGE (f : A -> B) t` THEN
    REWRITE_TAC [SUBSET; IN_IMAGE; EXTENSION] THEN
    STRIP_TAC THEN
    REPEAT CONJ_TAC THENL
    [POP_ASSUM (K ALL_TAC) THEN
     X_GEN_TAC `y : A` THEN
     DISCH_THEN (X_CHOOSE_THEN `x : B` STRIP_ASSUME_TAC) THEN
     FIRST_X_ASSUM (MP_TAC o SPEC `x : B`) THEN
     ASM_REWRITE_TAC [] THEN
     STRIP_TAC;
     POP_ASSUM (K ALL_TAC) THEN
     ...
     DISCH_THEN
       (CONJUNCTS_THEN2
          (X_CHOOSE_THEN `y1 : B` STRIP_ASSUME_TAC)
          (CONJUNCTS_THEN2
             (X_CHOOSE_THEN `y2 : B` STRIP_ASSUME_TAC)
             STRIP_ASSUME_TAC)) THEN
     REPEAT (FIRST_X_ASSUM SUBST_VAR_TAC) THEN
     AP_TERM_TAC THEN
     POP_ASSUM MP_TAC THEN
     FIRST_ASSUM (MP_TAC o SPEC `y2 : B`) THEN
     FIRST_X_ASSUM (MP_TAC o SPEC `y1 : B`) THEN
     ASM_REWRITE_TAC [] THEN
     STRIP_TAC THEN
     STRIP_TAC THEN
     ASM_REWRITE_TAC [];
     ...
     [STRIP_TAC THEN
      ...
      DISCH_THEN
        (X_CHOOSE_THEN `x : A`
           (CONJUNCTS_THEN2 SUBST_VAR_TAC
              (X_CHOOSE_THEN `z : B` STRIP_ASSUME_TAC))) THEN
      FIRST_X_ASSUM SUBST_VAR_TAC THEN
      FIRST_X_ASSUM (MP_TAC o SPEC `z : B`) THEN
      ASM_REWRITE_TAC [] THEN
      STRIP_TAC THEN
      ASM_REWRITE_TAC []]]]]);;

export_thm SUBSET_IMAGE_INJ;;
...
let EXISTS_SUBSET_IMAGE_INJ = prove
 (`!p (f : A -> B) s.
  ...
  REPEAT GEN_TAC THEN
  EQ_TAC THENL
  [REWRITE_TAC [SUBSET_IMAGE_INJ] THEN
   DISCH_THEN
     (X_CHOOSE_THEN `t : B set`
        (CONJUNCTS_THEN2
           (X_CHOOSE_THEN `u : A set` STRIP_ASSUME_TAC)
           STRIP_ASSUME_TAC)) THEN
   FIRST_X_ASSUM SUBST_VAR_TAC THEN
   EXISTS_TAC `u : A set` THEN
   ASM_REWRITE_TAC [];
...
export_thm EXISTS_SUBSET_IMAGE_INJ;;
...
let FORALL_SUBSET_IMAGE_INJ = prove
 (`!p (f : A -> B) s.
  ...
  MATCH_MP_TAC (TAUT `!x y. (~x <=> ~y) ==> (x <=> y)`) THEN
  REWRITE_TAC
    [NOT_FORALL_THM; NOT_IMP; EXISTS_SUBSET_IMAGE_INJ; GSYM CONJ_ASSOC]);;

export_thm FORALL_SUBSET_IMAGE_INJ;;

let IMAGE_INTER_SUBSET = prove
 (`!(f : A -> B) s t. IMAGE f (s INTER t) SUBSET IMAGE f s INTER IMAGE f t`,
  REPEAT GEN_TAC THEN
  REWRITE_TAC [SUBSET; IN_IMAGE; IN_INTER] THEN
  X_GEN_TAC `y : B` THEN
  DISCH_THEN (X_CHOOSE_THEN `x : A` STRIP_ASSUME_TAC) THEN
  FIRST_X_ASSUM SUBST_VAR_TAC THEN
  CONJ_TAC THEN
...
let IMAGE_INTER_SATURATED_GEN = prove
 (`!(f : A -> B) s t u.
      ...
      IMAGE f (s INTER t) = IMAGE f s INTER IMAGE f t`,
  REPEAT GEN_TAC THEN
  REWRITE_TAC [SUBSET; EXTENSION; IN_IMAGE; IN_INTER; IN_ELIM] THEN
  STRIP_TAC THENL
  [X_GEN_TAC `y : B` THEN
   EQ_TAC THENL
   [DISCH_THEN (X_CHOOSE_THEN `x : A` STRIP_ASSUME_TAC) THEN
    ...
    DISCH_THEN
      (CONJUNCTS_THEN2
         (X_CHOOSE_THEN `x1 : A` STRIP_ASSUME_TAC)
         (X_CHOOSE_THEN `x2 : A` STRIP_ASSUME_TAC) o
       ONCE_REWRITE_RULE [EQ_SYM_EQ]) THEN
    FIRST_X_ASSUM (MP_TAC o SPEC `x2 : A`) THEN
    ASM_REWRITE_TAC [] THEN
    STRIP_TAC THEN
    FIRST_X_ASSUM (MP_TAC o SPEC `x2 : A`) THEN
    ASM_REWRITE_TAC [] THEN
    ANTS_TAC THENL
    [EXISTS_TAC `x1 : A` THEN
     ASM_REWRITE_TAC [];
     STRIP_TAC THEN
     EXISTS_TAC `x2 : A` THEN
     ASM_REWRITE_TAC []]];
   X_GEN_TAC `y : B` THEN
   EQ_TAC THENL
   [DISCH_THEN (X_CHOOSE_THEN `x : A` STRIP_ASSUME_TAC) THEN
    ...
    DISCH_THEN
      (CONJUNCTS_THEN2
         ...
         (X_CHOOSE_THEN `x2 : A` STRIP_ASSUME_TAC) o
       ONCE_REWRITE_RULE [EQ_SYM_EQ]) THEN
    FIRST_X_ASSUM (MP_TAC o SPEC `x1 : A`) THEN
    ASM_REWRITE_TAC [] THEN
    STRIP_TAC THEN
    FIRST_X_ASSUM (MP_TAC o SPEC `x1 : A`) THEN
    ASM_REWRITE_TAC [] THEN
...
export_thm IMAGE_INTER_SATURATED_GEN;;
...
export_thm IMAGE_INTER_SATURATED;;
...
let INTER_INTERS = prove
 (`!(s : A set) f.
  ...
  REPEAT STRIP_TAC THEN
  COND_CASES_TAC THENL
  [ASM_REWRITE_TAC [INTERS_0; INTER_UNIV];
   REWRITE_TAC [INTERS_GSPEC] THEN
   REWRITE_TAC [EXTENSION; IN_INTER; IN_ELIM; IN_INTERS] THEN
   X_GEN_TAC `x : A` THEN
   BOOL_CASES_TAC `(x : A) IN s` THEN
   ASM_REWRITE_TAC [NOT_FORALL_THM; MEMBER_NOT_EMPTY]]);;

export_thm INTER_INTERS;;

let INTERS_INTER = prove
 (`!f (s : A set).
     INTERS f INTER s =
     if f = {} then s else INTERS {t INTER s | t IN f}`,
  REPEAT GEN_TAC THEN
  ONCE_REWRITE_TAC [INTER_COMM] THEN
  MATCH_ACCEPT_TAC INTER_INTERS);;

export_thm INTERS_INTER;;
...
let UNIONS_OVER_INTERS = prove
 (`!(f : A -> (B set) set) s.
  ...
  REWRITE_TAC [IN_INTERS; IN_ELIM] THEN
...
export_thm UNIONS_OVER_INTERS;;

let IMAGE_INTERS_SUBSET = prove
 (`!(f : A -> B) s. IMAGE f (INTERS s) SUBSET INTERS (IMAGE (IMAGE f) s)`,
  REPEAT GEN_TAC THEN
  REWRITE_TAC [INTERS_IMAGE; SUBSET; IN_ELIM; IN_IMAGE] THEN
  X_GEN_TAC `y : B` THEN
...
export_thm IMAGE_INTERS_SUBSET;;
 ...
 (`!(f : A -> B) g s u.
  ...
  SUBGOAL_THEN
    `!g.
        ~(g = {}) /\
        (!t. t IN g ==> t SUBSET u) /\
        (!t. t IN g ==> {x | x IN u /\ f x IN IMAGE f t} SUBSET t) ==>
        IMAGE (f : A -> B) (INTERS g) = INTERS (IMAGE (IMAGE f) g)`
     ASSUME_TAC THENL
  [REPEAT STRIP_TAC THEN
   ONCE_REWRITE_TAC [EXTENSION] THEN
   X_GEN_TAC `y : B` THEN
   ...
   [DISCH_THEN (X_CHOOSE_THEN `x : A` STRIP_ASSUME_TAC) THEN
    ...
    ASM_REWRITE_TAC [];
    STRIP_TAC THEN
    FIRST_X_ASSUM (MP_TAC o CONV_RULE (REWR_CONV (GSYM MEMBER_NOT_EMPTY))) THEN
    DISCH_THEN (X_CHOOSE_THEN `t : A set` STRIP_ASSUME_TAC) THEN
    FIRST_ASSUM (MP_TAC o SPEC `t : A set`) THEN
    ANTS_TAC THEN
    ASM_REWRITE_TAC [IN_IMAGE] THEN
    DISCH_THEN (X_CHOOSE_THEN `x : A` STRIP_ASSUME_TAC) THEN
    FIRST_X_ASSUM SUBST_VAR_TAC THEN
    ...
    X_GEN_TAC `v : A set` THEN
    STRIP_TAC THEN
    FIRST_X_ASSUM (MP_TAC o SPEC `v : A set`) THEN
    ASM_REWRITE_TAC [] THEN
    STRIP_TAC THEN
    FIRST_X_ASSUM (MP_TAC o SPEC `v : A set`) THEN
    ...
    ASM_REWRITE_TAC []];
   ALL_TAC] THEN
  REVERSE_TAC (ASM_CASES_TAC `(s : A set) IN g`) THENL
  [POP_ASSUM (SUBST1_TAC o REWRITE_RULE [GSYM DELETE_NON_ELEMENT]) THEN
   FIRST_X_ASSUM MATCH_ACCEPT_TAC;
   ALL_TAC] THEN
  STRIP_TAC THEN
  UNDISCH_TAC `!t : A set. t IN g ==> t SUBSET u` THEN
  MP_TAC (ISPECL [`s : A set`; `g : (A set) set`] INSERT_DELETE) THEN
  ASM_REWRITE_TAC [] THEN
  DISCH_THEN (SUBST1_TAC o SYM) THEN
  ASM_CASES_TAC `g DELETE (s : A set) = {}` THENL
  [POP_ASSUM SUBST1_TAC THEN
   REWRITE_TAC [IN_SING; INTERS_1; IMAGE_SING];
   ALL_TAC] THEN
  POP_ASSUM MP_TAC THEN
  POP_ASSUM MP_TAC THEN
  POP_ASSUM (K ALL_TAC) THEN
  POP_ASSUM (K ALL_TAC) THEN
  SPEC_TAC (`g DELETE (s : A set)`, `h : (A set) set`) THEN
  REPEAT STRIP_TAC THEN
  REWRITE_TAC [INTERS_INSERT; IMAGE_INSERT] THEN
  MATCH_MP_TAC EQ_TRANS THEN
  EXISTS_TAC `IMAGE (f : A -> B) s INTER IMAGE f (INTERS h)` THEN
  ...
  DISJ2_TAC THEN
  CONJ_TAC THENL
  [REWRITE_TAC [SUBSET_INTERS] THEN
   X_GEN_TAC `t : A set` THEN
   STRIP_TAC THEN
   MATCH_MP_TAC SUBSET_TRANS THEN
   EXISTS_TAC `{x | x IN u /\ (f : A -> B) x IN IMAGE f t}` THEN
   CONJ_TAC THENL
   [REWRITE_TAC [SUBSET; IN_ELIM] THEN
    X_GEN_TAC `x : A` THEN
    STRIP_TAC THEN
    ASM_REWRITE_TAC [] THEN
    MP_TAC
      (ISPECL [`f : A -> B`; `(INTERS h) : A set`; `t : A set`]
         IMAGE_SUBSET) THEN
    ANTS_TAC THENL
...
export_thm IMAGE_INTERS_SATURATED_GEN;;
...
let IMAGE_INTERS_SATURATED = prove
 (`!(f : A -> B) g s.
    ...
    IMAGE f (INTERS g) = INTERS (IMAGE (IMAGE f) g)`,
  REPEAT STRIP_TAC THEN
  MATCH_MP_TAC IMAGE_INTERS_SATURATED_GEN THEN
  MAP_EVERY EXISTS_TAC [`s : A set`; `UNIV : A set`] THEN
  ASM_REWRITE_TAC [IN_UNIV; SUBSET_UNIV]);;

export_thm IMAGE_INTERS_SATURATED;;
...
let INFINITE_IMAGE_INJ = prove
 (`!(f:A->B) s.
  ...
  REWRITE_TAC [INFINITE] THEN
  STRIP_TAC THEN
  MP_TAC (ISPECL [`f : A -> B`; `s : A set`] FINITE_IMAGE_INJ_EQ) THEN
  ASM_REWRITE_TAC []);;
...
let EXISTS_FINITE_SUBSET_IMAGE_INJ = prove
 (`!p (f : A -> B) s.
  ...
  REWRITE_TAC [EXISTS_SUBSET_IMAGE_INJ] THEN
  AP_TERM_TAC THEN
  ABS_TAC THEN
  AP_TERM_TAC THEN
  ...
  AP_THM_TAC THEN
  AP_TERM_TAC THEN
  MATCH_MP_TAC (TAUT `!x y z. (x ==> (y <=> z)) ==> (x /\ y <=> z /\ x)`) THEN
  REWRITE_TAC [GSYM CONJ_ASSOC] THEN
  MATCH_ACCEPT_TAC FINITE_IMAGE_INJ_EQ);;

export_thm EXISTS_FINITE_SUBSET_IMAGE_INJ;;
...
let FORALL_FINITE_SUBSET_IMAGE_INJ = prove
 (`!p (f : A -> B) s.
  ...
  MATCH_MP_TAC (TAUT `!x y. (~x <=> ~y) ==> (x <=> y)`) THEN
  REWRITE_TAC
    [NOT_FORALL_THM; NOT_IMP; EXISTS_FINITE_SUBSET_IMAGE_INJ;
     GSYM CONJ_ASSOC]);;

export_thm FORALL_FINITE_SUBSET_IMAGE_INJ;;
...
let FINITE_SUBSET_UNIONS = prove
 (`!f (s : A set).
      ...
      ?g. FINITE g /\ g SUBSET f /\ s SUBSET (UNIONS g)`,
  REPEAT STRIP_TAC THEN
  FIRST_X_ASSUM(MP_TAC o GEN_REWRITE_RULE I [SUBSET]) THEN
  GEN_REWRITE_TAC (LAND_CONV o TOP_DEPTH_CONV)
   [IN_UNIONS; RIGHT_IMP_EXISTS_THM] THEN
  REWRITE_TAC[SKOLEM_THM; LEFT_IMP_EXISTS_THM] THEN
  X_GEN_TAC `u : A -> A set` THEN DISCH_TAC THEN
  EXISTS_TAC `IMAGE (u : A -> A set) s` THEN
  ASM_SIMP_TAC[FINITE_IMAGE; UNIONS_IMAGE] THEN
  REWRITE_TAC [SUBSET; IN_ELIM; IN_IMAGE] THEN
  CONJ_TAC THENL
  [X_GEN_TAC `t : A set` THEN
   STRIP_TAC THEN
   FIRST_X_ASSUM (MP_TAC o SPEC `x : A`) THEN
...
export_thm FINITE_SUBSET_UNIONS;;
...
let UNIONS_IN_CHAIN = prove
 (`!f : (A set) set.
  ...
  MAP_EVERY X_GEN_TAC [`s : A set`; `f : (A set) set`] THEN
  ASM_CASES_TAC `f : (A set) set = {}` THEN
  ASM_REWRITE_TAC[UNIONS_0; IN_INSERT; UNION_EMPTY] THEN
  DISCH_THEN(fun th -> STRIP_TAC THEN MP_TAC th) THEN
  ASM_REWRITE_TAC[] THEN
  SPEC_TAC (`UNIONS (f : (A set) set)`, `t : A set`) THEN
  GEN_TAC THEN
  MATCH_MP_TAC (MESON []
    `((t : A set) IN f ==> s UNION t = s \/ s UNION t = t) ==>
     (t IN f ==> s UNION t = s \/ s UNION t IN f)`) THEN
  REWRITE_TAC
    [GSYM SUBSET_UNION_ABSORPTION;
     ONCE_REWRITE_RULE [UNION_COMM] (GSYM SUBSET_UNION_ABSORPTION)] THEN
  FIRST_X_ASSUM MATCH_ACCEPT_TAC);;

export_thm UNIONS_IN_CHAIN;;
...
let INTERS_IN_CHAIN = prove
 (`!f : (A set) set.
  ...
  DISCH_THEN(fun th -> STRIP_TAC THEN MP_TAC th) THEN
  ASM_REWRITE_TAC[] THEN
  SPEC_TAC (`INTERS (f : (A set) set)`, `t : A set`) THEN
  GEN_TAC THEN
  MATCH_MP_TAC (MESON []
    `((t : A set) IN f ==> s INTER t = s \/ s INTER t = t) ==>
     (t IN f ==> s INTER t = s \/ s INTER t IN f)`) THEN
  REWRITE_TAC
    [GSYM SUBSET_INTER_ABSORPTION;
     ONCE_REWRITE_RULE [INTER_COMM] (GSYM SUBSET_INTER_ABSORPTION)] THEN
  FIRST_X_ASSUM MATCH_ACCEPT_TAC);;

export_thm INTERS_IN_CHAIN;;
...
let FINITE_SUBSET_UNIONS_CHAIN = prove
 (`!f (s : A set).
        ...
        ==> ?t. t IN f /\ s SUBSET t`,
  REPEAT STRIP_TAC THEN
  MP_TAC(ISPECL [`f : (A set) set`; `s : A set`]
        FINITE_SUBSET_UNIONS) THEN
  ASM_REWRITE_TAC[LEFT_IMP_EXISTS_THM] THEN
  X_GEN_TAC `g : (A set) set` THEN
  ASM_CASES_TAC `g : (A set) set = {}` THENL
   [ASM_SIMP_TAC [UNIONS_0] THEN
    REWRITE_TAC [SUBSET_EMPTY; EMPTY_SUBSET; FINITE_EMPTY] THEN
    DISCH_THEN SUBST1_TAC THEN
    ASM_REWRITE_TAC [EMPTY_SUBSET; MEMBER_NOT_EMPTY];
    STRIP_TAC] THEN
  EXISTS_TAC `UNIONS g : A set` THEN
  ASM_REWRITE_TAC[] THEN
  FIRST_ASSUM(MATCH_MP_TAC o REWRITE_RULE[SUBSET]) THEN
  MATCH_MP_TAC UNIONS_IN_CHAIN THEN
  ASM_REWRITE_TAC [] THEN
  X_GEN_TAC `t : A set` THEN
  X_GEN_TAC `u : A set` THEN
  STRIP_TAC THEN
  FIRST_X_ASSUM MATCH_MP_TAC THEN
  UNDISCH_TAC `(g : (A set) set) SUBSET f` THEN
  REWRITE_TAC [SUBSET] THEN
  STRIP_TAC THEN
  CONJ_TAC THEN
  FIRST_X_ASSUM MATCH_MP_TAC THEN
...
export_thm FINITE_SUBSET_UNIONS_CHAIN;;
...
let CHOOSE_SUBSET_EQ = prove
 (`!n (s : A set).
  ...
  DISCH_THEN(X_CHOOSE_THEN `t : A set` STRIP_ASSUME_TAC) THEN DISCH_TAC THEN
  UNDISCH_TAC `(t : A set) HAS_SIZE n` THEN
  REWRITE_TAC [HAS_SIZE] THEN
  STRIP_TAC THEN
  FIRST_X_ASSUM SUBST_VAR_TAC THEN
  MATCH_MP_TAC CARD_SUBSET THEN
...
let CARD_LE_UNIONS_CHAIN = prove
 (`!(f : (A set) set) n.
  ...
  REPEAT GEN_TAC THEN ASM_CASES_TAC `f : (A set) set = {}` THEN
  ...
  ONCE_REWRITE_TAC[SWAP_EXISTS_THM] THEN
  MATCH_MP_TAC MONO_EXISTS THEN X_GEN_TAC `s:A set` THEN
  REWRITE_TAC[HAS_SIZE] THEN STRIP_TAC THEN ASM_REWRITE_TAC[] THEN
  MATCH_MP_TAC FINITE_SUBSET_UNIONS_CHAIN THEN
  ASM_REWRITE_TAC[]);;

export_thm CARD_LE_UNIONS_CHAIN;;

let CARD_LE_1 = prove
 (`!s : A set. FINITE s /\ CARD s <= 1 <=> ?a. s SUBSET {a}`,
  GEN_TAC THEN
  SUBGOAL_THEN `!n : num. n <= 1 <=> n = 0 \/ n = 1`
    (fun th -> REWRITE_TAC [th]) THENL
  [INDUCT_TAC THENL
   [NUM_REDUCE_TAC;
    REWRITE_TAC [ONE; NOT_SUC; SUC_INJ; LE_SUC; LE_ZERO]];
   REWRITE_TAC[LEFT_OR_DISTRIB; GSYM HAS_SIZE] THEN
   CONV_TAC(ONCE_DEPTH_CONV HAS_SIZE_CONV) THEN
   REWRITE_TAC [SUBSET; NOT_IN_EMPTY; IN_SING] THEN
   EQ_TAC THENL
   [STRIP_TAC THEN
    ASM_REWRITE_TAC [NOT_IN_EMPTY; IN_SING] THEN
    EXISTS_TAC `a : A` THEN
    REWRITE_TAC [];
    ...
    [ASM_REWRITE_TAC [];
     DISCH_THEN SUBST1_TAC THEN
     FIRST_X_ASSUM (MP_TAC o SPEC `b : A`) THEN
     ASM_REWRITE_TAC [] THEN
     DISCH_THEN (SUBST1_TAC o SYM) THEN
     ASM_REWRITE_TAC []]]]);;

export_thm CARD_LE_1;;
...
let EMPTY_CROSS = prove
 (`!s. {} CROSS s = ({} : (A # B) set)`,
  REWRITE_TAC [CROSS_EQ_EMPTY]);;

export_thm EMPTY_CROSS;;
...
let SUBSET_CROSS = prove
 (`!(s : A set) (t : B set) s' t'.
  ...
  REPEAT GEN_TAC THEN
  REWRITE_TAC [SUBSET; FORALL_IN_CROSS; IN_CROSS] THEN
  ASM_CASES_TAC `(s : A set) = {}` THENL
  [POP_ASSUM SUBST_VAR_TAC THEN
  ...
  ASM_REWRITE_TAC [] THEN
  POP_ASSUM
    (X_CHOOSE_THEN `sx : A` ASSUME_TAC o
     REWRITE_RULE [GSYM MEMBER_NOT_EMPTY]) THEN
  ASM_CASES_TAC `(t : B set) = {}` THENL
  [POP_ASSUM SUBST_VAR_TAC THEN
   REWRITE_TAC [NOT_IN_EMPTY];
   ALL_TAC] THEN
  ASM_REWRITE_TAC [] THEN
  POP_ASSUM
    (X_CHOOSE_THEN `tx : B` ASSUME_TAC o
     REWRITE_RULE [GSYM MEMBER_NOT_EMPTY]) THEN
  REVERSE_TAC EQ_TAC THENL
  [REPEAT STRIP_TAC THEN
   FIRST_X_ASSUM MATCH_MP_TAC THEN
   ...
   ALL_TAC] THEN
  REPEAT STRIP_TAC THENL
  [FIRST_X_ASSUM (MP_TAC o SPECL [`x : A`; `tx : B`]) THEN
   ASM_REWRITE_TAC [] THEN
   STRIP_TAC;
   FIRST_X_ASSUM (MP_TAC o SPECL [`sx : A`; `x : B`]) THEN
   ASM_REWRITE_TAC [] THEN
   STRIP_TAC]);;

export_thm SUBSET_CROSS;;

let CROSS_EQ = prove
 (`!(s1 : A set) (t1 : B set) s2 t2.
     s1 CROSS t1 = s2 CROSS t2 <=>
     (s1 = {} \/ t1 = {}) /\ (s2 = {} \/ t2 = {}) \/ s1 = s2 /\ t1 = t2`,
  REPEAT GEN_TAC THEN
  REWRITE_TAC [GSYM SUBSET_ANTISYM_EQ; SUBSET_CROSS] THEN
  REWRITE_TAC [EMPTY_SUBSET; SUBSET_EMPTY] THEN
  ASM_CASES_TAC `(s1 : A set) = {}` THEN
  ASM_REWRITE_TAC [EMPTY_SUBSET; SUBSET_EMPTY] THEN
  ASM_CASES_TAC `(s2 : A set) = {}` THEN
  ASM_REWRITE_TAC [EMPTY_SUBSET; SUBSET_EMPTY] THEN
  ASM_CASES_TAC `(t1 : B set) = {}` THEN
  ASM_REWRITE_TAC [EMPTY_SUBSET; SUBSET_EMPTY] THEN
  ASM_CASES_TAC `(t2 : B set) = {}` THEN
  ASM_REWRITE_TAC [EMPTY_SUBSET; SUBSET_EMPTY] THEN
  ITAUT_TAC);;
...
let IMAGE_FST_CROSS = prove
 (`!(s : A set) (t : B set).
  ...
  X_GEN_TAC `x : A` THEN
  ONCE_REWRITE_TAC [CONJ_SYM] THEN
  REWRITE_TAC [EXISTS_IN_CROSS; FST] THEN
  EQ_TAC THENL
  [STRIP_TAC THEN
   ASM_REWRITE_TAC [];
   STRIP_TAC THEN
...
let IMAGE_SND_CROSS = prove
 (`!(s : A set) (t : B set).
  ...
  REPEAT GEN_TAC THEN
  COND_CASES_TAC THEN
  ASM_REWRITE_TAC [EMPTY_CROSS; IMAGE_CLAUSES] THEN
  REWRITE_TAC [EXTENSION; IN_IMAGE] THEN
  X_GEN_TAC `y : B` THEN
  ONCE_REWRITE_TAC [CONJ_SYM] THEN
  REWRITE_TAC [EXISTS_IN_CROSS; SND] THEN
  EQ_TAC THENL
  [STRIP_TAC THEN
   ASM_REWRITE_TAC [];
   STRIP_TAC THEN
...
export_thm IMAGE_SND_CROSS;;

let INTER_CROSS = prove
 (`!(s : B set) (t : A set) u.
     (t INTER u) CROSS s = (t CROSS s) INTER (u CROSS s)`,
...
export_thm CROSS_INTER;;

let INTER_CROSS_INTER = prove
 (`!(s1 : A set) s2 (t1 : B set) t2.
     (s1 CROSS t1) INTER (s2 CROSS t2) = (s1 INTER s2) CROSS (t1 INTER t2)`,
  REWRITE_TAC [EXTENSION; IN_INTER; FORALL_PAIR_THM; IN_CROSS] THEN
  CONV_TAC TAUT);;

export_thm INTER_CROSS_INTER;;

let UNION_CROSS = prove
 (`!(s : B set) (t : A set) u.
     (t UNION u) CROSS s = (t CROSS s) UNION (u CROSS s)`,
  REWRITE_TAC [EXTENSION; FORALL_PAIR_THM; IN_UNION; IN_CROSS] THEN
  REPEAT STRIP_TAC THEN
  CONV_TAC TAUT);;

export_thm UNION_CROSS;;
...
export_thm CROSS_UNION;;

let DIFF_CROSS = prove
 (`!(s : B set) (t : A set) u.
     (t DIFF u) CROSS s = (t CROSS s) DIFF (u CROSS s)`,
  REWRITE_TAC [EXTENSION; FORALL_PAIR_THM; IN_DIFF; IN_CROSS] THEN
  REPEAT STRIP_TAC THEN
  CONV_TAC TAUT);;

export_thm DIFF_CROSS;;
...
export_thm CROSS_DIFF;;

let CROSS_UNIONS = prove
 (`!(s : A set) (f : (B set) set).
     s CROSS UNIONS f = UNIONS {s CROSS t | t IN f}`,
  REPEAT GEN_TAC THEN
  ONCE_REWRITE_TAC [EXTENSION] THEN
  REWRITE_TAC [IN_UNIONS; FORALL_PAIR_THM; IN_CROSS; EXISTS_IN_GSPEC] THEN
  X_GEN_TAC `a : A` THEN
  X_GEN_TAC `b : B` THEN
  BOOL_CASES_TAC `(a : A) IN s` THEN
  REWRITE_TAC []);;

export_thm CROSS_UNIONS;;

let UNIONS_CROSS = prove
 (`!(f : (A set) set) (t : B set).
  ...
  X_GEN_TAC `a : A` THEN
  X_GEN_TAC `b : B` THEN
  BOOL_CASES_TAC `(b : B) IN t` THEN
  REWRITE_TAC []);;

export_thm UNIONS_CROSS;;

let UNIONS_CROSS_UNIONS = prove
 (`!(f : (A set) set) (g : (B set) set).
      UNIONS f CROSS UNIONS g = UNIONS {s CROSS t | s IN f /\ t IN g}`,
  REPEAT GEN_TAC THEN
  ONCE_REWRITE_TAC [EXTENSION] THEN
  REWRITE_TAC
    [IN_UNIONS; FORALL_PAIR_THM; IN_CROSS; UNIONS_CROSS; CROSS_UNIONS;
     EXISTS_IN_GSPEC] THEN
  REPEAT GEN_TAC THEN
  EQ_TAC THENL
  [DISCH_THEN
     (CONJUNCTS_THEN2
        (X_CHOOSE_THEN `s : A set` STRIP_ASSUME_TAC)
        (X_CHOOSE_THEN `t : B set` STRIP_ASSUME_TAC)) THEN
   EXISTS_TAC `s : A set` THEN
   EXISTS_TAC `t : B set` THEN
   ASM_REWRITE_TAC [];
   DISCH_THEN
     (X_CHOOSE_THEN `s : A set`
        (X_CHOOSE_THEN `t : B set` STRIP_ASSUME_TAC)) THEN
   CONJ_TAC THENL
   [EXISTS_TAC `s : A set` THEN
    ASM_REWRITE_TAC [];
...
export_thm UNIONS_CROSS_UNIONS;;

let CROSS_INTERS = prove
 (`!(s : A set) (f : (B set) set).
      s CROSS (INTERS f) =
      if f = {} then s CROSS UNIV else INTERS {s CROSS t | t IN f}`,
  REPEAT STRIP_TAC THEN
  COND_CASES_TAC THENL
  [ASM_REWRITE_TAC [INTERS_0];
   REWRITE_TAC [INTERS_GSPEC] THEN
   REWRITE_TAC [EXTENSION; FORALL_PAIR_THM; IN_CROSS] THEN
   X_GEN_TAC `x : A` THEN
   X_GEN_TAC `y : B` THEN
   REWRITE_TAC [IN_ELIM_THM; EXISTS_PAIR_THM; PAIR_EQ; GSYM CONJ_ASSOC] THEN
   ONCE_REWRITE_TAC [SWAP_EXISTS_THM] THEN
   REWRITE_TAC [UNWIND_THM1; IN_CROSS; IN_INTERS] THEN
   BOOL_CASES_TAC `(x : A) IN s` THEN
   ASM_REWRITE_TAC [NOT_FORALL_THM; MEMBER_NOT_EMPTY]]);;

export_thm CROSS_INTERS;;

let INTERS_CROSS = prove
 (`!(f : (A set) set) (s : B set).
      (INTERS f) CROSS s =
      if f = {} then UNIV CROSS s else INTERS {t CROSS s | t IN f}`,
  REPEAT STRIP_TAC THEN
  COND_CASES_TAC THENL
  [ASM_REWRITE_TAC [INTERS_0];
   REWRITE_TAC [INTERS_GSPEC] THEN
   ...
   ONCE_REWRITE_TAC [SWAP_EXISTS_THM] THEN
   REWRITE_TAC [UNWIND_THM1; IN_CROSS; IN_INTERS] THEN
   BOOL_CASES_TAC `(y : B) IN s` THEN
   ASM_REWRITE_TAC [NOT_FORALL_THM; MEMBER_NOT_EMPTY]]);;

export_thm INTERS_CROSS;;

let INTERS_CROSS_INTERS = prove
 (`!(f : (A set) set) (g : (B set) set).
  ...
  [ASM_REWRITE_TAC [INTERS_0] THEN
   ...
   REWRITE_TAC [IN_ELIM_THM; EXISTS_PAIR_THM; PAIR_EQ; GSYM CONJ_ASSOC] THEN
   ONCE_REWRITE_TAC [SWAP_EXISTS_THM] THEN
   REWRITE_TAC [UNWIND_THM1; IN_CROSS; IN_INTERS; IN_UNIV];
   ASM_REWRITE_TAC [INTERS_0] THEN
   REWRITE_TAC [INTERS_GSPEC] THEN
   REWRITE_TAC [EXTENSION; FORALL_PAIR_THM; IN_CROSS] THEN
   X_GEN_TAC `x : A` THEN
   ...
   [REPEAT STRIP_TAC THEN
    ...
    [X_GEN_TAC `s : A set` THEN
     STRIP_TAC THEN
     UNDISCH_TAC `~((g : (B set) set) = {})` THEN
     REWRITE_TAC [GSYM MEMBER_NOT_EMPTY] THEN
     DISCH_THEN (X_CHOOSE_THEN `t : B set` STRIP_ASSUME_TAC) THEN
     FIRST_X_ASSUM (MP_TAC o SPECL [`s : A set`; `t : B set`]) THEN
     ASM_REWRITE_TAC [] THEN
     STRIP_TAC;
     X_GEN_TAC `t : B set` THEN
     STRIP_TAC THEN
     UNDISCH_TAC `~((f : (A set) set) = {})` THEN
     REWRITE_TAC [GSYM MEMBER_NOT_EMPTY] THEN
     DISCH_THEN (X_CHOOSE_THEN `s : A set` STRIP_ASSUME_TAC) THEN
     FIRST_X_ASSUM (MP_TAC o SPECL [`s : A set`; `t : B set`]) THEN
     ASM_REWRITE_TAC [] THEN
     STRIP_TAC]]]);;
...
let FINITE_CROSS_EQ = prove
 (`!(s : A set) (t : B set).
  ...
  REPEAT GEN_TAC THEN
  ASM_CASES_TAC `s : A set = {}` THEN
  ASM_REWRITE_TAC [EMPTY_CROSS; FINITE_EMPTY] THEN
  ASM_CASES_TAC `t : B set = {}` THEN
  ASM_REWRITE_TAC [CROSS_EMPTY; FINITE_EMPTY] THEN
  EQ_TAC THEN
  REWRITE_TAC [FINITE_CROSS] THEN
  POP_ASSUM
    (X_CHOOSE_THEN `y : B` MP_TAC o
     REWRITE_RULE [GSYM MEMBER_NOT_EMPTY]) THEN
  POP_ASSUM
    (X_CHOOSE_THEN `x : A` MP_TAC o
     REWRITE_RULE [GSYM MEMBER_NOT_EMPTY]) THEN
  REPEAT STRIP_TAC THENL
  [MATCH_MP_TAC FINITE_SUBSET THEN
   EXISTS_TAC `IMAGE (FST : A # B -> A) (s CROSS t)` THEN
   CONJ_TAC THENL
   [MATCH_MP_TAC FINITE_IMAGE THEN
    ASM_REWRITE_TAC [];
    REWRITE_TAC [SUBSET; IN_IMAGE] THEN
    X_GEN_TAC `z : A` THEN
    STRIP_TAC THEN
    EXISTS_TAC `(z,y) : A # B` THEN
    ASM_REWRITE_TAC [IN_CROSS]];
   MATCH_MP_TAC FINITE_SUBSET THEN
   EXISTS_TAC `IMAGE (SND : A # B -> B) (s CROSS t)` THEN
   CONJ_TAC THENL
   [MATCH_MP_TAC FINITE_IMAGE THEN
    ASM_REWRITE_TAC [];
    REWRITE_TAC [SUBSET; IN_IMAGE] THEN
...
let FINITE_POWERSET_EQ = prove
 (`!s:A set. FINITE {t | t SUBSET s} <=> FINITE s`,
  ...
  SUBGOAL_THEN `FINITE(IMAGE (\x:A. {x}) s)` MP_TAC THENL
   [FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ]
        FINITE_SUBSET)) THEN
    SIMP_TAC[SUBSET; FORALL_IN_IMAGE; IN_ELIM; IN_SING];
...
let FINITE_RESTRICTED_FUNSPACE = prove
 (`!s : A set t : B set k.
  ...
  EXISTS_TAC
   `IMAGE (\u:(A#B)set x. if ?y. (x,y) IN u then @y. (x,y) IN u else k x)
  ...
  [REWRITE_TAC[SUBSET; FORALL_IN_IMAGE; IN_ELIM_THM; IN_CROSS] THEN
   REWRITE_TAC [FUN_EQ_THM] THEN
   X_GEN_TAC `x : A` THEN
   REWRITE_TAC [IN_IMAGE; IN_ELIM; PAIR_EQ; GSYM CONJ_ASSOC; UNWIND_THM1] THEN
   COND_CASES_TAC THENL
   [POP_ASSUM (MP_TAC o SELECT_RULE) THEN
    STRIP_TAC THEN
    ASM_REWRITE_TAC [];
    POP_ASSUM (MP_TAC o SPEC `(f : A -> B) x` o
               REWRITE_RULE [NOT_EXISTS_THM]) THEN
    REWRITE_TAC []] THEN
   NO_TAC;
   ONCE_REWRITE_TAC [UNWIND_THM1] THEN
   POP_ASSUM MP_TAC THEN
   REWRITE_TAC [SUBSET; FORALL_IN_IMAGE; IN_ELIM_THM; IN_CROSS; UNWIND_THM1] THEN
   STRIP_TAC THEN
   X_GEN_TAC `x : A` THEN
   STRIP_TAC THEN
   FIRST_X_ASSUM (MP_TAC o SPEC `x : A`) THEN
   ...
   STRIP_TAC THEN
   ASM_REWRITE_TAC [] THEN
   FIRST_X_ASSUM (MATCH_MP_TAC o CONV_RULE (REWR_CONV SUBSET)) THEN
   MATCH_MP_TAC FUN_IN_IMAGE THEN
   ASM_REWRITE_TAC []]);;

export_thm FINITE_RESTRICTED_FUNSPACE;;
...
let num_INFINITE_EQ = prove
 (`!s : num set. INFINITE s <=> !N. ?n. N <= n /\ n IN s`,
  GEN_TAC THEN REWRITE_TAC[INFINITE; num_FINITE] THEN
  REWRITE_TAC
    [NOT_EXISTS_THM; NOT_FORALL_THM; NOT_IMP; NOT_LE; GSYM LE_SUC_LT] THEN
  EQ_TAC THENL
  [STRIP_TAC THEN
...
let INJECTIVE_ON_PREIMAGE = prove
 (`!(f : A -> B) s u.
      (!t1 t2.
         t1 SUBSET u /\ t2 SUBSET u /\
         {x | x IN s /\ f x IN t1} = {x | x IN s /\ f x IN t2} ==>
         t1 = t2) <=>
      u SUBSET IMAGE f s`,
  REPEAT GEN_TAC THEN
  EQ_TAC THENL
  [STRIP_TAC THEN
   ...
   X_GEN_TAC `y : B` THEN
   STRIP_TAC THEN
   FIRST_X_ASSUM (MP_TAC o SPECL [`{y : B}`; `{} : B set`]) THEN
   REWRITE_TAC [SUBSET; EXTENSION] THEN
   ASM_REWRITE_TAC
     [IN_SING; NOT_IN_EMPTY; IN_ELIM; FORALL_UNWIND_THM2; IN_IMAGE] THEN
   CONV_TAC (REWR_CONV (GSYM CONTRAPOS_THM)) THEN
   REWRITE_TAC [NOT_EXISTS_THM; NOT_FORALL_THM; NOT_IMP; EXISTS_REFL] THEN
   STRIP_TAC THEN
   X_GEN_TAC `x : A` THEN
   STRIP_TAC THEN
   FIRST_X_ASSUM (MP_TAC o SPEC `x : A`) THEN
   ASM_REWRITE_TAC [];
   REWRITE_TAC [EXTENSION; SUBSET; IN_ELIM; IN_IMAGE] THEN
   STRIP_TAC THEN
   REPEAT GEN_TAC THEN
   STRIP_TAC THEN
   X_GEN_TAC `y : B` THEN
   EQ_TAC THENL
   [STRIP_TAC THEN
    FIRST_X_ASSUM (K ALL_TAC o SPEC `y : B`) THEN
    FIRST_X_ASSUM (MP_TAC o SPEC `y : B`) THEN
    ASM_REWRITE_TAC [] THEN
    STRIP_TAC THEN
    FIRST_X_ASSUM (MP_TAC o SPEC `y : B`) THEN
...
export_thm INJECTIVE_ON_PREIMAGE;;
 ...
 (`!(f : A -> B) s u.
  ...
  [STRIP_TAC THEN
   ...
   [REWRITE_TAC [SUBSET; FORALL_IN_IMAGE] THEN
    ...
    DISCH_TAC THEN
    FIRST_X_ASSUM (MP_TAC o SPEC `{x : A}`) THEN
    ASM_REWRITE_TAC [SING_SUBSET; EXTENSION; IN_SING; IN_ELIM] THEN
    REWRITE_TAC [SUBSET] THEN
    DISCH_THEN (X_CHOOSE_THEN `t : B set` STRIP_ASSUME_TAC) THEN
    FIRST_X_ASSUM (MP_TAC o SPEC `x : A`) THEN
    ASM_REWRITE_TAC [];
    ...
    ASM_REWRITE_TAC [SING_SUBSET; EXTENSION; IN_SING; IN_ELIM] THEN
    DISCH_THEN (X_CHOOSE_THEN `t : B set` STRIP_ASSUME_TAC) THEN
    FIRST_ASSUM (MP_TAC o SPEC `y : A`) THEN
    FIRST_X_ASSUM (MP_TAC o SPEC `x : A`) THEN
    ASM_REWRITE_TAC [] THEN
    STRIP_TAC THEN
    ASM_REWRITE_TAC [] THEN
   ...
   [MATCH_MP_TAC SUBSET_TRANS THEN
    ...
    MATCH_MP_TAC IMAGE_SUBSET THEN
    ASM_REWRITE_TAC [];
    POP_ASSUM (STRIP_ASSUME_TAC o REWRITE_RULE [SUBSET]) THEN
    REWRITE_TAC [EXTENSION; IN_ELIM] THEN
    X_GEN_TAC `x : A` THEN
    EQ_TAC THENL
    [REWRITE_TAC [IN_IMAGE] THEN
     DISCH_THEN
       (CONJUNCTS_THEN2 STRIP_ASSUME_TAC
          (X_CHOOSE_THEN `y : A` STRIP_ASSUME_TAC)) THEN
     SUBGOAL_THEN `(x : A) = y` (fun th -> ASM_REWRITE_TAC [th]) THEN
     FIRST_X_ASSUM MATCH_MP_TAC THEN
     ASM_REWRITE_TAC [] THEN
...
export_thm SURJECTIVE_ON_PREIMAGE;;
...
let INJECTIVE_PREIMAGE = prove
 (`!(f : A -> B).
  ...
  REWRITE_TAC [IN_UNIV; SUBSET_UNIV; UNIV_SUBSET]);;

export_thm INJECTIVE_PREIMAGE;;
...
export_thm SURJECTIVE_PREIMAGE;;
