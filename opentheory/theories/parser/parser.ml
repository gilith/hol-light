...
let parser_all_def = new_definition
  `!a s. parser_all (a : A) (s : A pstream) = SOME (a,s)`;;
...
let is_parser_all = prove
  (`is_parser (parser_all : A -> A pstream -> (A # A pstream) option)`,
   REWRITE_TAC
     [is_parser_def; parser_all_def; case_option_def; is_suffix_pstream_refl]);;
...
let parse_parse_all_cons = prove
  (`!(x : A) s. parse parse_all (ConsPstream x s) = SOME (x,s)`,
    REPEAT GEN_TAC THEN
    ASM_REWRITE_TAC [parse_def; dest_parse_all; parser_all_def]);;

export_thm parse_parse_all_cons;;
...
let parse_parse_all = prove
  (`parse (parse_all : (A,A) parser) =
    ...
    STRIP_TAC THEN
    ASM_REWRITE_TAC
      [parse_error; parse_eof; parse_parse_all_cons; case_pstream_def]);;
...
let parse_all_inverse = prove
  (`parse_inverse (parse_all : (A,A) parser) (\a. CONS a [])`,
   ...
   REPEAT STRIP_TAC THEN
   ASM_REWRITE_TAC
     [parse_parse_all; append_pstream_def; case_pstream_def;
      case_option_def]);;
...
let parse_all_strong_inverse = prove
  (`parse_strong_inverse (parse_all : (A,A) parser) (\a. CONS a [])`,
   ...
   ASM_REWRITE_TAC [parse_parse_all; append_pstream_def] THEN
   REPEAT GEN_TAC THEN
   MP_TAC (ISPEC `s : A pstream` pstream_cases) THEN
   STRIP_TAC THEN
   ASM_REWRITE_TAC
...
let is_parser_partial_map = prove
  (`!f p.
      is_parser (parser_partial_map (f : B -> C option) (p : (A,B) parser))`,
   REPEAT GEN_TAC THEN
...
let parse_partial_map_inverse = prove
  (`!f p g e.
   ...
   REPEAT STRIP_TAC THEN
   ASM_REWRITE_TAC
     [parse_parse_partial_map; append_pstream_def; case_pstream_def;
      case_option_def]);;
...
let parse_partial_map_strong_inverse = prove
  (`!(f : B -> C option) p g e.
   ...
   [MATCH_MP_TAC parse_partial_map_inverse THEN
    ...
    ASM_REWRITE_TAC [parse_parse_partial_map; append_pstream_def] THEN
