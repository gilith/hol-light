...
let mod_le_word10_size = new_axiom
  `!n. n MOD word10_size <= n`;;
...
let num_to_word10_to_num = new_axiom
  `!n. word10_to_num (num_to_word10 n) = n MOD word10_size`;;

let num_to_word10_to_num_bound = new_axiom
  `!n. n < word10_size ==> word10_to_num (num_to_word10 n) = n`;;
...
new_constant ("random_word10", `:random -> word10`);;

let random_word10_def = new_axiom
  `!r. random_word10 r = num_to_word10 (random_uniform word10_size r)`;;
...
let num_to_word10_exp = new_axiom
   `!m n. num_to_word10 (m EXP n) = word10_exp (num_to_word10 m) n`;;

let word10_zero_exp = new_axiom
   `!n.
      word10_exp (num_to_word10 0) n =
      if n = 0 then num_to_word10 1 else num_to_word10 0`;;

let word10_exp_add = new_axiom
   `!x m n.
      word10_mult (word10_exp x m) (word10_exp x n) =
      word10_exp x (m + n)`;;

let word10_exp_one = new_axiom
   `!x. word10_exp x 1 = x`;;
...
let word10_le_trans = new_axiom
  `!x1 x2 x3. word10_le x1 x2 /\ word10_le x2 x3 ==> word10_le x1 x3`;;

let word10_lt_refl = new_axiom
  `!x. ~word10_lt x x`;;

let word10_lte_trans = new_axiom
  `!x1 x2 x3. word10_lt x1 x2 /\ word10_le x2 x3 ==> word10_lt x1 x3`;;

let word10_let_trans = new_axiom
  `!x1 x2 x3. word10_le x1 x2 /\ word10_lt x2 x3 ==> word10_lt x1 x3`;;

let word10_lt_trans = new_axiom
  `!x1 x2 x3. word10_lt x1 x2 /\ word10_lt x2 x3 ==> word10_lt x1 x3`;;

new_constant ("word10_shl", `:word10 -> num -> word10`);;

let word10_shl_def = new_axiom
  `!w n. word10_shl w n = num_to_word10 (bit_shl (word10_to_num w) n)`;;

new_constant ("word10_shr", `:word10 -> num -> word10`);;

let word10_shr_def = new_axiom
  `!w n. word10_shr w n = num_to_word10 (bit_shr (word10_to_num w) n)`;;

new_constant ("word10_bit", `:word10 -> num -> bool`);;

let word10_bit_def = new_axiom
  `!w n. word10_bit w n = bit_nth (word10_to_num w) n`;;

new_constant ("word10_to_list", `:word10 -> bool list`);;

let word10_to_list_def = new_axiom
  `!w. word10_to_list w = num_to_bitvec (word10_to_num w) word10_width`;;
...
let bit_bound_word10_to_num = new_axiom
  `!w. bit_bound (word10_to_num w) word10_width = word10_to_num w`;;

let num_to_word10_to_num_bit_bound = new_axiom
  `!n. word10_to_num (num_to_word10 n) = bit_bound n word10_width`;;
...
let bit_width_word10_to_num = new_axiom
  `!w. bit_width (word10_to_num w) <= word10_width`;;

let word10_to_list_to_num = new_axiom
  `!w. bits_to_num (word10_to_list w) = word10_to_num w`;;
...
let word10_lte_cmp = new_axiom
  `!q l1 l2.
     LENGTH l1 = LENGTH l2 ==>
     word10_bits_lte q l1 l2 = bit_cmp q (bits_to_num l1) (bits_to_num l2)`;;
