...
let bmap_bsub_imp = prove
 (`!f x k n y. bsub x k n y ==> bsub (bmap f x) k n (bmap f y)`,
  ...
  [REWRITE_TAC [bmap_width; ADD_0] THEN
   MATCH_MP_TAC bsub_width THEN
...
let bzip_width = prove
 (`!f x y z.
     ...
     ?n.
       ...
       width z = n`,
  REPEAT STRIP_TAC THEN
  MP_TAC (SPECL [`f : wire -> wire -> wire`;
                 `x : bus`;
                 `y : bus`] bzip_def) THEN
  ASM_REWRITE_TAC [] THEN
  DISCH_THEN SUBST1_TAC THEN
  REWRITE_TAC [width_def; bus_tybij] THEN
  MATCH_MP_TAC length_zipwith THEN
  ASM_REWRITE_TAC [GSYM width_def]);;

export_thm bzip_width;;

let bzip_width_out = prove
 (`!f x y z n.
     bzip f x y z /\
     width x = n ==>
     width z = n`,
...
export_thm bzip_width_out;;

let bzip_bnil = prove
 (`!f. bzip f bnil bnil = bnil`,
  REPEAT STRIP_TAC THEN
  MP_TAC (SPECL [`f : wire -> wire -> wire`;
                 `bnil`;
                 `bnil`] bzip_def) THEN
  ASM_REWRITE_TAC [] THEN
  DISCH_THEN SUBST1_TAC THEN
  REWRITE_TAC [bnil_def; bus_tybij; zipwith_nil]);;

export_thm bzip_bnil;;

let bzip_bwire = prove
 (`!f w x. bzip f (bwire w) (bwire x) = bwire (f w x)`,
  REPEAT STRIP_TAC THEN
  MP_TAC (SPECL [`f : wire -> wire -> wire`;
                 `bwire w`;
                 `bwire x`] bzip_def) THEN
  ASM_REWRITE_TAC [bwire_width] THEN
  DISCH_THEN SUBST1_TAC THEN
  REWRITE_TAC [bwire_def; bus_tybij; zipwith_sing]);;

export_thm bzip_bwire;;

let bzip_bappend = prove
 (`!f x1 x2 y1 y2.
     width x1 = width y1 /\
     width x2 = width y2 ==>
     bzip f (bappend x1 x2) (bappend y1 y2) =
     bappend (bzip f x1 y1) (bzip f x2 y2)`,
  REPEAT STRIP_TAC THEN
  MP_TAC (SPECL [`f : wire -> wire -> wire`;
                 `x1 : bus`;
                 `y1 : bus`] bzip_def) THEN
  ASM_REWRITE_TAC [] THEN
  DISCH_THEN SUBST1_TAC THEN
  MP_TAC (SPECL [`f : wire -> wire -> wire`;
                 `x2 : bus`;
                 `y2 : bus`] bzip_def) THEN
  ASM_REWRITE_TAC [] THEN
  DISCH_THEN SUBST1_TAC THEN
  MP_TAC (SPECL [`f : wire -> wire -> wire`;
                 `bappend x1 x2`;
                 `bappend y1 y2`] bzip_def) THEN
  ASM_REWRITE_TAC [bappend_width] THEN
  DISCH_THEN SUBST1_TAC THEN
  REWRITE_TAC [bappend_def; bus_tybij; mk_bus_inj] THEN
  MATCH_MP_TAC zipwith_append THEN
  ASM_REWRITE_TAC [GSYM width_def]);;

export_thm bzip_bappend;;
