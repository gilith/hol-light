...
let mod_le_word16_size = new_axiom
  `!n. n MOD word16_size <= n`;;
...
let word16_exp_def = CONJ word16_exp_zero word16_exp_suc;;
...
new_constant ("random_word16", `:random -> word16`);;

let random_word16_def = new_axiom
  `!r. random_word16 r = num_to_word16 (random_uniform word16_size r)`;;
...
let word16_not_lt = new_axiom
   `!x y. ~(word16_lt x y) <=> word16_le y x`;;

let word16_not_le = new_axiom
   `!x y. ~(word16_le x y) <=> word16_lt y x`;;
...
let list_to_word16_def = new_axiom
  `!l. list_to_word16 l = num_to_word16 (bits_to_num l)`;;
...
let word16_and_def = new_axiom
  `!w1 w2.
      word16_and w1 w2 =
      num_to_word16 (bit_and (word16_to_num w1) (word16_to_num w2))`;;
...
let word16_or_def = new_axiom
  `!w1 w2.
      word16_or w1 w2 =
      num_to_word16 (bit_or (word16_to_num w1) (word16_to_num w2))`;;
...
let list_to_word16_cons = new_axiom
   `!h t.
...
let bit_bound_word16_to_num = new_axiom
  `!w. bit_bound (word16_to_num w) word16_width = word16_to_num w`;;

let num_to_word16_to_num_bit_bound = new_axiom
...
let word16_bit_and = new_axiom
  `!k w1 w2. word16_bit (word16_and w1 w2) k <=> word16_bit w1 k /\ word16_bit w2 k`;;

let word16_and_list = new_axiom
  `!w1 w2.
     word16_and w1 w2 =
     list_to_word16 (zipwith ( /\ ) (word16_to_list w1) (word16_to_list w2))`;;

let word16_bit_or = new_axiom
  `!k w1 w2. word16_bit (word16_or w1 w2) k <=> word16_bit w1 k \/ word16_bit w2 k`;;

let word16_or_list = new_axiom
  `!w1 w2.
     word16_or w1 w2 =
     list_to_word16 (zipwith ( \/ ) (word16_to_list w1) (word16_to_list w2))`;;

let word16_bit_not = new_axiom
  `!k w. word16_bit (word16_not w) k <=> k < word16_width /\ ~word16_bit w k`;;
