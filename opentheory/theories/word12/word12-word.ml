...
let num_to_word12_to_num = new_axiom
  `!n. word12_to_num (num_to_word12 n) = n MOD word12_size`;;

let num_to_word12_to_num_bound = new_axiom
  `!n. n < word12_size ==> word12_to_num (num_to_word12 n) = n`;;
...
new_constant ("word12_exp", `:word12 -> num -> word12`);;

let word12_exp_zero = new_axiom
  `!x. word12_exp x 0 = num_to_word12 1`;;

let word12_exp_suc = new_axiom
  `!x n. word12_exp x (SUC n) = word12_mult x (word12_exp x n)`;;

let word12_exp_def = CONJ word12_exp_zero word12_exp_suc;;
...
new_constant ("random_word12", `:random -> word12`);;

let random_word12_def = new_axiom
  `!r. random_word12 r = num_to_word12 (random_uniform word12_size r)`;;
...
let word12_not_lt = new_axiom
   `!x y. ~(word12_lt x y) <=> word12_le y x`;;

let word12_not_le = new_axiom
   `!x y. ~(word12_le x y) <=> word12_lt y x`;;
...
let num_to_word12_exp = new_axiom
   `!m n. num_to_word12 (m EXP n) = word12_exp (num_to_word12 m) n`;;

let word12_zero_exp = new_axiom
   `!n.
      word12_exp (num_to_word12 0) n =
      if n = 0 then num_to_word12 1 else num_to_word12 0`;;

let word12_exp_add = new_axiom
   `!x m n.
      word12_mult (word12_exp x m) (word12_exp x n) =
      word12_exp x (m + n)`;;

let word12_exp_one = new_axiom
   `!x. word12_exp x 1 = x`;;
...
let word12_le_trans = new_axiom
  `!x1 x2 x3. word12_le x1 x2 /\ word12_le x2 x3 ==> word12_le x1 x3`;;

let word12_lt_refl = new_axiom
  `!x. ~word12_lt x x`;;

let word12_lte_trans = new_axiom
  `!x1 x2 x3. word12_lt x1 x2 /\ word12_le x2 x3 ==> word12_lt x1 x3`;;

let word12_let_trans = new_axiom
  `!x1 x2 x3. word12_le x1 x2 /\ word12_lt x2 x3 ==> word12_lt x1 x3`;;
...
let word12_shl_def = new_axiom
  `!w n. word12_shl w n = num_to_word12 (bit_shl (word12_to_num w) n)`;;

new_constant ("word12_shr", `:word12 -> num -> word12`);;

let word12_shr_def = new_axiom
  `!w n. word12_shr w n = num_to_word12 (bit_shr (word12_to_num w) n)`;;

new_constant ("word12_bit", `:word12 -> num -> bool`);;

let word12_bit_def = new_axiom
  `!w n. word12_bit w n = bit_nth (word12_to_num w) n`;;

new_constant ("word12_to_list", `:word12 -> bool list`);;

let word12_to_list_def = new_axiom
  `!w. word12_to_list w = num_to_bitvec (word12_to_num w) word12_width`;;

new_constant ("list_to_word12", `:bool list -> word12`);;

let list_to_word12_def = new_axiom
  `!l. list_to_word12 l = num_to_word12 (bits_to_num l)`;;
...
let word12_and_def = new_axiom
  `!w1 w2.
      word12_and w1 w2 =
      num_to_word12 (bit_and (word12_to_num w1) (word12_to_num w2))`;;
...
let word12_or_def = new_axiom
  `!w1 w2.
      word12_or w1 w2 =
      num_to_word12 (bit_or (word12_to_num w1) (word12_to_num w2))`;;
...
let list_to_word12_cons = new_axiom
   `!h t.
      list_to_word12 (CONS h t) =
      if h then word12_add (num_to_word12 1) (word12_shl (list_to_word12 t) 1)
      else word12_shl (list_to_word12 t) 1`;;

let bit_bound_word12_to_num = new_axiom
  `!w. bit_bound (word12_to_num w) word12_width = word12_to_num w`;;

let num_to_word12_to_num_bit_bound = new_axiom
  `!n. word12_to_num (num_to_word12 n) = bit_bound n word12_width`;;
...
let bit_width_word12_to_num = new_axiom
  `!w. bit_width (word12_to_num w) <= word12_width`;;

let word12_to_list_to_num = new_axiom
  `!w. bits_to_num (word12_to_list w) = word12_to_num w`;;
...
let list_to_word12_to_list_eq = new_axiom
  `!l.
     ...
     if LENGTH l <= word12_width then
       APPEND l (REPLICATE F (word12_width - LENGTH l))
     else
       take word12_width l`;;
...
let word12_shl_list = new_axiom
  `!l n.
     ...
     list_to_word12 (APPEND (REPLICATE F n) l)`;;
...
let word12_eq_bits = new_axiom
  `!w1 w2.
     (!i. i < word12_width ==> word12_bit w1 i = word12_bit w2 i) ==> w1 = w2`;;

let word12_lte_cmp = new_axiom
  `!q l1 l2.
     LENGTH l1 = LENGTH l2 ==>
     word12_bits_lte q l1 l2 = bit_cmp q (bits_to_num l1) (bits_to_num l2)`;;
...
let list_to_word12_to_list_conv =
  ...
  cond_conv
    ...
    (RAND_CONV
       (RAND_CONV
          (LAND_CONV word12_width_conv THENC
...
let numeral_to_word12_list_conv =
  let list_to_word12_conv = REWR_CONV (GSYM list_to_word12_def) in
  RAND_CONV numeral_to_bits_conv THENC
  list_to_word12_conv;;

let word12_and_list_conv =
  let th = SPECL [`list_to_word12 l1`; `list_to_word12 l2`] word12_and_list in
  REWR_CONV th THENC
  RAND_CONV
    (LAND_CONV list_to_word12_to_list_conv THENC
     RAND_CONV list_to_word12_to_list_conv THENC
     zipwith_conv and_simp_conv);;

let word12_or_list_conv =
  let th = SPECL [`list_to_word12 l1`; `list_to_word12 l2`] word12_or_list in
  REWR_CONV th THENC
  RAND_CONV
    (LAND_CONV list_to_word12_to_list_conv THENC
     RAND_CONV list_to_word12_to_list_conv THENC
...
let bit_blast_conv = DEPTH_CONV bit_blast_subterm_conv;;  (* word12 *)
let bit_blast_tac = CONV_TAC bit_blast_conv;;  (* word12 *)
