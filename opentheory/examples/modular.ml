...
export_thm zero_modular_add;;

(*PARAMETRIC
let zero_modular_add = new_axiom
   `!x. modular_add (num_to_modular 0) x = x`;;
*)

let modular_add_zero = prove
  (`!x. modular_add x (num_to_modular 0) = x`,
   GEN_TAC THEN
   ONCE_REWRITE_TAC [modular_add_comm] THEN
   MATCH_ACCEPT_TAC zero_modular_add);;

export_thm modular_add_zero;;

(*PARAMETRIC
let modular_add_zero = new_axiom
   `!x. modular_add x (num_to_modular 0) = x`;;
*)
...
let modular_add_neg = prove
  (`!x. modular_add x (modular_neg x) = num_to_modular 0`,
   REPEAT GEN_TAC THEN
   ONCE_REWRITE_TAC [modular_add_comm] THEN
   MATCH_ACCEPT_TAC modular_neg_add);;

export_thm modular_add_neg;;

(*PARAMETRIC
let modular_add_neg = new_axiom
   `!x. modular_add x (modular_neg x) = num_to_modular 0`;;
*)
...
let modular_add_left_cancel = prove
  (`!x y z. modular_add x y = modular_add x z <=> y = z`,
   ...
   EQ_TAC THENL
   [STRIP_TAC THEN
    ONCE_REWRITE_TAC [GSYM zero_modular_add] THEN
    ONCE_REWRITE_TAC [GSYM (SPEC `x : modular` modular_neg_add)] THEN
...
let modular_neg_neg = prove
  (`!x. modular_neg (modular_neg x) = x`,
   ...
   REWRITE_TAC [modular_add_neg; modular_neg_add]);;
...
export_thm zero_modular_mult;;

(*PARAMETRIC
let zero_modular_mult = new_axiom
   `!x. modular_mult (num_to_modular 0) x = num_to_modular 0`;;
*)

let modular_mult_zero = prove
  (`!x. modular_mult x (num_to_modular 0) = num_to_modular 0`,
   GEN_TAC THEN
   ONCE_REWRITE_TAC [modular_mult_comm] THEN
   MATCH_ACCEPT_TAC zero_modular_mult);;

export_thm modular_mult_zero;;

(*PARAMETRIC
let modular_mult_zero = new_axiom
   `!x. modular_mult x (num_to_modular 0) = num_to_modular 0`;;
*)

let one_modular_mult = prove
...
export_thm one_modular_mult;;

(*PARAMETRIC
let one_modular_mult = new_axiom
   `!x. modular_mult (num_to_modular 1) x = x`;;
*)

let modular_mult_one = prove
  (`!x. modular_mult x (num_to_modular 1) = x`,
   GEN_TAC THEN
   ONCE_REWRITE_TAC [modular_mult_comm] THEN
   MATCH_ACCEPT_TAC one_modular_mult);;

export_thm modular_mult_one;;

(*PARAMETRIC
let modular_mult_one = new_axiom
   `!x. modular_mult x (num_to_modular 1) = x`;;
*)

let modular_neg_mult = prove
  (`!x y. modular_mult (modular_neg x) y = modular_neg (modular_mult x y)`,
   ...
   REWRITE_TAC [modular_add_neg] THEN
   REWRITE_TAC
     [GSYM modular_add_right_distrib; modular_add_neg; zero_modular_mult]);;

export_thm modular_neg_mult;;

(*PARAMETRIC
let modular_neg_mult = new_axiom
   `!x y. modular_mult (modular_neg x) y = modular_neg (modular_mult x y)`;;
*)

let modular_mult_neg = prove
  (`!x y. modular_mult x (modular_neg y) = modular_neg (modular_mult x y)`,
   REPEAT GEN_TAC THEN
   ONCE_REWRITE_TAC [modular_mult_comm] THEN
   MATCH_ACCEPT_TAC modular_neg_mult);;

export_thm modular_mult_neg;;

(*PARAMETRIC
let modular_mult_neg = new_axiom
   `!x y. modular_mult x (modular_neg y) = modular_neg (modular_mult x y)`;;
*)
