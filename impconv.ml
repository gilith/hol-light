...
(* A few constants. *)
let A_ = `A:bool` and B_ = `B:bool` and C_ = `X:bool` and D_ = `D:bool` in
let T_ = `T:bool` in
...
let MKIMPL_CONTRA_IFF =
  let lem = TAUT `((A ==> B) ==> X) ==> (A <=> B) ==> X /\ (B ==> A)` in
  fun th ->
...
let MKIMP_IMP_CONTRA_CTXT =
  let lem = TAUT `(B==>A) /\ (A==>B==>X==>D) ==> (A==>X) ==> (B==>D)` in
  fun th1 th2 ->
...
let MKIMP_IMP_CO_CTXT =
  let lem = TAUT `(A==>B) /\ (A==>B==>D==>X) ==> (B==>D) ==> (A==>X)` in
  fun th1 th2 ->
...
let MKIMP_CONJ_CONTRA_CTXT =
  let lem = TAUT `(X==>A==>B) /\ (A==>B==>X==>D) ==> (A/\X==>B/\D)` in
  fun th1 th2 ->
    let a,bcd = dest_imp (concl th2) in
    let b,cd = dest_imp bcd in
    let c,d = dest_imp cd in
...
let MKIMP_CONJ_CO_CTXT =
  let lem = TAUT `(B==>A) /\ (B==>D==>X) ==> (B/\D==>A/\X)` in
  fun th1 th2 ->
...
let rec REWR_IMPCONV_OF_CONV =
  let IMP_SYM = REWR_RULE (TAUT `A==>B==>X <=> B==>A==>X`) in
